...
- **Thread list + Command Palette polish (sub-project 4)**: conversations gain `pinned: bool` and `frozen_at: float | None` plus a new `PATCH /api/conversations/{id}` (one-way freeze; `append_turn` returns 409 when frozen). Frontend `ThreadList` rewritten with title-only filter, per-row `MoreMenu` (pin/unpin, freeze, rename, duplicate, delete, export JSON), Pin/Lock badges, and a separate `Checkpoints` section that holds frozen conversations (hidden when empty). `Composer` swaps the textarea for a frozen banner with a Duplicate CTA when the active conversation is frozen. `CommandPalette` gains two dynamic groups — `Conversations` (top 50 non-frozen, sorted by recency) and `This conversation` (Pin/Freeze/Rename/Duplicate/Delete/Export, with Freeze hidden when already frozen) — wired through a new `executeCommand` helper that bypasses the registry for non-registered dynamic commands. Store gains `refreshConversations`, `setConversationPinned`, `freezeConversation`, `duplicateConversation`, `renameConversation`, `deleteConversationRemote` with optimistic-update + revert-on-error. Spec: `docs/superpowers/specs/2026-04-19-thread-list-palette-polish-design.md`. (`backend/app/api/conversations_api.py`, `frontend/src/lib/api-backend.ts`, `frontend/src/lib/store.ts`, `frontend/src/components/shell/ThreadList.tsx`, `frontend/src/components/chat/composer/Composer.tsx`, `frontend/src/components/command-palette/CommandPalette.tsx`) — `1d13020`, `2a64a48`, `a21556e`, `c0c7407`, `fe52edd`, `28e2510`, `5736885`
- **Dock + Artifact Viewer (sub-project 3)**: right-side `Dock` is now a thin orchestrator over three first-class panels — `DockProgress` (per-step expand-row with timeline/context/raw segmented control inside each step), `DockContext` (per-conversation budget bar with err/warn/ok tones, layer breakdown, loaded files with unload, attached files, todos, scratchpad preview), and `DockArtifacts` (grid/list toggle with per-format lucide icons, dispatching `focusArtifact`). New full-viewport `ArtifactViewer` modal (createPortal + focus-trap-react) renders all six artifact formats via lazy renderers (`TextRenderer`, `HtmlRenderer` iframe-srcDoc-sandbox, `TableRenderer`, `CsvRenderer` (papaparse), `VegaLiteRenderer` (vega-embed), `MermaidRenderer`); keyboard nav: ←/→ cycle, ⌘C copy, ⌘S download, ESC close. Standalone `#/artifact/:id` hash route (`ArtifactPage`) for "open in new window". `useUiStore` v2 schema additions: `progressExpanded`, `artifactView`, `recentCommandIds`, `traceTab`. Retires `cockpit/TraceRail.tsx` and `lib/right-rail-store.ts` (folded into `useUiStore.traceTab`); `Dock` moved from `components/shell/` to `components/dock/`. New deps: `papaparse`, `focus-trap-react`. (`frontend/src/components/dock/`, `frontend/src/components/artifact/`, `frontend/src/lib/hooks/useArtifactNav.ts`, `frontend/src/routes/ArtifactPage.tsx`, `frontend/src/App.tsx`) — `7b99bf9`, `08cfea7`, `284e2e5`
- **Chat surface rebuild (sub-project 2)**: thread-header toolbar (`TitleEditor`, `SearchButton` ⌘K, `ForkButton`, `ExportMenu` md/json/html, `ProgressToggle` pulse, `MoreMenu` rename/duplicate/delete), card `Composer` with IconRow + `ModelPicker` + `ExtendedToggle` + `PlanModeToggle` + ⌘↵ + Send/Stop, and avatar-bubble `Message` renderer with `Callout`s, `ToolChipRow` (dispatches `scrollToTrace`), `ArtifactPillRow` (dispatches `focusArtifact` + opens dock), `SubagentCard`s, and `AttachedFileChip`s. New `ChatPane` replaces `ChatMain` as the chat-section entry point. Data-model extensions: per-conversation `model` / `extendedThinking` / `attachedFiles`, new `ToolCallLogEntry.messageId`/`rows`, `callout` `ContentBlock` variant, and store actions for `deleteConversation` + `setThreadsOpen`. Shortcuts: `CYCLE_MODEL` (⌘⇧M) and `TOGGLE_EXTENDED` (⌘⇧E). Retires `ChatInput.tsx`, `MessageBubble.tsx`, `cockpit/ChatMain.tsx`, `ChatLayout.tsx`, `SessionLayout.tsx`, and orphan `ChatSection.tsx`. (`frontend/src/components/chat/ChatPane.tsx`, `frontend/src/components/chat/header/`, `frontend/src/components/chat/composer/`, `frontend/src/components/chat/message/`, `frontend/src/lib/store.ts`, `frontend/src/lib/ui-store.ts`, `frontend/src/lib/shortcuts.ts`, `frontend/src/App.tsx`, `frontend/e2e/chat-surface.spec.ts`)
- **Post-v2 coherence — Wave D**: final wave of the umbrella sweep. (1) E2E Playwright coverage: new `frontend/e2e/panels.spec.ts` exercises all five right-rail drawers (DIGEST, HEALTH, SKILLS, GRAPH, INGEST) — topbar stack rendering, open/close cycle, graph empty-state copy, ingest dropzone+input focus, and the HEALTH → DIGEST BUILD subsection header. Follows the `loadApp()` swallow-backend-failures precedent from `smoke.spec.ts`. Ships `pnpm test:e2e` and `make e2e-panels`. (2) GAN trace corpus bridge: `scripts/gan-trace-corpus.py --list|--load <id>` exposes the YAML trace store as JSON over stdout, with `$TRACE_DIR` fallback resolution. Ships `docs/gan-trace-corpus.md` and 5 round-trip unit tests. (3) Graph↔wiki drift detection: new `backend/app/integrity/drift_scan.py::scan_drift(cfg)` returns a frozen `DriftReport(timestamp, total, by_kind, findings)` with three rules — `orphan_claim`, `orphan_backlink`, `stale_claim` (30-day default) — that never raises when the KB is unavailable. `scripts/sb-drift-scan.py` writes `<repo>/telemetry/drift/YYYY-MM-DD.json`. New `GET /api/sb/drift?date=…` route returns `{report: null}` when missing and `{report: null, error: 'malformed_report'}` on corrupt JSON; 404 when `SECOND_BRAIN_ENABLED=False`. HealthPanel gains a `DRIFT · today` section showing `orphan claims / orphan backlinks / stale claims` counts with a muted empty state. (`frontend/e2e/panels.spec.ts`, `scripts/gan-trace-corpus.py`, `scripts/sb-drift-scan.py`, `backend/app/integrity/drift_scan.py`, `backend/app/api/sb_api.py`, `frontend/src/components/health/HealthPanel.tsx`, `frontend/src/lib/health-store.ts`, `docs/gan-trace-corpus.md`) — `b811dd6`, `81181d6`, `4d7a941`
- **Post-v2 coherence — sweep complete**: all 10 umbrella items shipped across Waves A–D (panels: DIGEST-pending, memory recall, HEALTH; telemetry: digest build costs, skills usage; graph+ingest: GRAPH viz, INGEST drop-zone; observability: E2E panels spec, GAN corpus bridge, graph-wiki drift). Success criterion met: a user opening the app cold can see pending KB decisions, last session's memory recall, today's digest cost + health score, skill usage today, browse the graph, drop a file to ingest, and see wiki/claim drift — all without leaving the right rail. Spec: `docs/superpowers/specs/2026-04-18-post-v2-coherence.md`.

- **Post-v2 coherence — Wave C**: knowledge-graph viz and ingest drop-zone surfaces. (1) `GET /api/sb/graph?center=…&depth=…&limit=…` reads the `edges`/`claims`/`sources` tables in the property-graph DuckDB store via a monkeypatchable `_query_graph` seam; clamps depth to `[1,2]` and limit to `[1,200]`, degrades to `{nodes: [], edges: [], note: "no graph data"}` when the store is missing, unopenable, or lacks an `edges` table. New `GraphPanel` in topbar slot 3 renders a hand-rolled SVG force-directed layout (50 nodes max, spring-repulsion with seeded PRNG) with a `CENTER` input + `GO` button; clicking a node re-centers the graph. (2) `POST /api/sb/ingest` wraps the existing `sb_ingest` handler behind a pydantic `IngestBody`; new `IngestPanel` in topbar slot 4 ships a dropzone (falls back to manual path input since browsers hide real filesystem paths) + last-10 recent-ingests list; topbar badge counts failed ingests in the last minute. All new routes 404 when `SECOND_BRAIN_ENABLED=False`. (`backend/app/api/sb_api.py`, `frontend/src/lib/graph-store.ts`, `frontend/src/components/graph/`, `frontend/src/lib/ingest-store.ts`, `frontend/src/components/ingest/`, `frontend/src/App.tsx`) — `7dd94cd`, `c7687a9`
- **Post-v2 coherence — Wave B**: observability surfaces for digest builds and skill usage. (1) `POST /api/sb/digest/build` now writes a `{digests_dir}/YYYY-MM-DD.meta.json` telemetry sidecar (spec §4 shape — `actor/duration_ms/outcome/detail`) on both success and error, and a new `GET /api/sb/digest/costs?date=…` surfaces the record; `HealthPanel` gains a `DIGEST BUILD · today` section with a muted empty state. (2) `_load_skill_body` is instrumented to append one `{actor: skill:<name>, duration_ms, outcome, detail}` record per load to `telemetry/skills.jsonl` (best-effort — telemetry failures never break the skill load); `GET /api/skills/telemetry` reads it reverse-chronologically with a `limit` param (intentionally not gated on `SECOND_BRAIN_ENABLED`); a new `SkillsPanel` in slot 2 of the topbar aggregates events most-used-first with today's invocation count as the badge. (`backend/app/telemetry/`, `backend/app/api/sb_api.py`, `backend/app/api/skills_telemetry_api.py`, `backend/app/harness/skill_tools.py`, `frontend/src/lib/health-store.ts`, `frontend/src/components/health/HealthPanel.tsx`, `frontend/src/lib/skills-store.ts`, `frontend/src/components/skills/`, `frontend/src/App.tsx`) — `6bbe2fa`, `c42cea3`
- **Post-v2 coherence — Wave A**: three right-rail panels land behind a shared `TopbarButton` component. (1) `HealthPanel` (slot 1) surfaces the KB health score, grade, components, and raw stats over `GET /api/sb/stats`. (2) `DigestPanel` gains a PENDING tab fed by `GET /api/sb/digest/pending` with a `BUILD NOW` action wired to `POST /api/sb/digest/build`; topbar badge count now sums digest unread + pending proposals. (3) ContextInspector gains a KB RECALL section via `GET /api/sb/memory/session/{id}` (with `?prompt=` override) that runs the second-brain injection runner against the last user prompt and lists recalled claim ids. All routes return 404 when `SECOND_BRAIN_ENABLED=False`. (`frontend/src/components/ui/TopbarButton.tsx`, `frontend/src/components/health/`, `frontend/src/components/digest/DigestPanel.tsx`, `frontend/src/devtools/ContextInspector.tsx`, `backend/app/api/sb_api.py`) — `8e32a9a`, `eea4c8c`, `2fd3db3`, `0c9814e`
...
- **second-brain v2 digest**: daily digest pipeline with 5 passes (reconciliation, wiki_bridge, taxonomy_drift, stale_review, edge_audit) surfaced via `sb maintain --digest` + `sb digest {build,apply,skip,read,ls}`; `sb stats` gains a `digest_unread_penalty` and `sb eval digest` runs per-pass golden comparison. Nothing auto-mutates — digest proposes, user applies.
