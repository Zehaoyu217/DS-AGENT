...
def _kpi(**overrides):
    from app.skills.reporting.dashboard_builder.pkg.build import KPICard
...
def _kpi_section(**kw):
    from app.skills.reporting.dashboard_builder.pkg.build import SectionSpec

    return SectionSpec(kind="kpi", span=3, payload=_kpi(**kw))


def _spec(sections):
    from app.skills.reporting.dashboard_builder.pkg.build import DashboardSpec
...
def test_too_many_sections_raises() -> None:
    from app.skills.reporting.dashboard_builder.pkg.build import validate_spec
...
def test_unknown_layout_raises() -> None:
    from app.skills.reporting.dashboard_builder.pkg.build import DashboardSpec, validate_spec
